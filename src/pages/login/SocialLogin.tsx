import { MessageCircle } from 'lucide-react';

import BoogleLogo from './components/BoogleLogo';

// 소셜 로그인 페이지: 로고 + 카카오/구글 로그인 버튼
// 카카오/구글은 브랜드 고정색이라 공통 Button 대신 전용 마크업 사용.
interface SocialLoginPropTypes {
  onKakaoLogin: () => void;
  onGoogleLogin: () => void;
}

const SocialLogin = ({ onKakaoLogin, onGoogleLogin }: SocialLoginPropTypes) => {
  return (
    <div className="flex min-h-dvh flex-col bg-beige-5 px-layout pb-18">
      <div className="flex flex-1 flex-col items-center justify-center">
        <BoogleLogo className="text-[3rem] text-orange-5" />
        <p className="body-lg mt-4 text-gray-8">내 장 건강을 위한 작은 습관</p>
      </div>

      <div className="flex flex-col gap-3">
        <button
          type="button"
          onClick={onKakaoLogin}
          className="body-m flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-[#FEE500] text-gray-10 active:translate-y-px"
        >
          <MessageCircle className="h-5 w-5 fill-current" />
          카카오로 로그인
        </button>

        <button
          type="button"
          onClick={onGoogleLogin}
          className="body-m flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl border border-gray-4 bg-beige-1 text-gray-10 active:translate-y-px"
        >
          {/* Google 'G' 로고 */}
          <svg className="h-5 w-5" viewBox="0 0 48 48" aria-hidden="true">
            <path
              fill="#EA4335"
              d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5Z"
            />
            <path
              fill="#4285F4"
              d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65Z"
            />
            <path
              fill="#FBBC05"
              d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19Z"
            />
            <path
              fill="#34A853"
              d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48Z"
            />
          </svg>
          Google로 로그인
        </button>
      </div>
    </div>
  );
};

export default SocialLogin;
