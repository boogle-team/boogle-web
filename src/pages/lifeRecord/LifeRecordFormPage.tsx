...
type LifeRecordFormModeTypes = 'create' | 'edit';

const LifeRecordFormPage = () => {
  const { recordId } = useParams();
  const [searchParams] = useSearchParams();
  const date = searchParams.get('date');
  const formMode: LifeRecordFormModeTypes = recordId ? 'edit' : 'create';
  const title = formMode === 'edit' ? '생활 기록 수정' : '생활 기록 작성';
...
export default LifeRecordFormPage;
