...
import type { CalendarMarkTypes, CalendarRecordMapTypes } from '../types/calendarTypes';
...
const getMarksForDay = (day: number): CalendarMarkTypes[] => {
  ...
  if (day % 2 === 1 && day <= 20) return day === 13 ? ['boogle', 'life'] : ['boogle'];
  return [];
};

export const getMockCalendarRecords = (currentDate: Dayjs): CalendarRecordMapTypes => {
  const daysInMonth = currentDate.daysInMonth();

  return Array.from({ length: daysInMonth }, (_, index) => index + 1).reduce<CalendarRecordMapTypes>(
    (recordMap, day) => {
