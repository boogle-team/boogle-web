...
const BottomNavigation = () => {
  ...
  return (
    <>
      <nav
        ref={navRef}
        className="fixed bottom-0 left-1/2 z-50 h-[var(--bottom-navigation-shell-height)] w-full max-w-[430px] -translate-x-1/2 overflow-visible"
      >
        {/* 실제 렌더 폭에 맞춰 생성한 하단 네비게이션 실루엣 (노치 곡률은 폭과 무관하게 고정) */}
        {navWidth > 0 && (
          <svg
            className="absolute inset-x-0 bottom-0 z-10 h-[var(--bottom-navigation-bar-height)] w-full overflow-visible"
        ...
        {/* 네비게이션 아이템 */}
        <div className="absolute inset-x-0 bottom-0 z-20 grid h-[var(--bottom-navigation-bar-height)] grid-cols-[1fr_1fr_5rem_1fr_1fr] items-end gap-x-[1.06rem] px-[1.56rem] pb-[var(--bottom-navigation-content-padding-bottom)]">
